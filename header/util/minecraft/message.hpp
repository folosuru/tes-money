...
inline void sendTrlText(Player *player, Arrai18n::trl_text) {
    if (player != nullptr) {
        player->sendText(player->getLanguageCode(), trl_text);
    }
}

inline std::string trl(const CommandOrigin& origin, const trl_text& text) {
