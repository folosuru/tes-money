...
void serve() {
            ...
            tes::geometry::Area2D serve_area{15,20,45,60};
    {
        auto land_mng = std::make_shared<tes::LandManager>(*dominion);
        {
            ...
            auto i = land_mng->prepareServe(tes::PlayerIdentify(new tes::PlayerIdentifyClass("foo")),
                serve_area);
            ...
            cpptf::isTrue("access from dominion_land #1 ", [=]() -> bool{
                ...
                std::cout << land.land.id << std::endl;
            ...
            cpptf::isTrue("access from LandManager #1", [=]() -> bool{
                ...
                return std::get<tes::Types::land_area_ref>(land).get().land.id == serve_land_id;
            }());
        ...
        cpptf::isSame("already served land",tes::ServeLandOrder::ErrorCode::ServedLand,
            [land_mng] {
                ...
                auto i = land_mng->prepareServe(tes::PlayerIdentify(new tes::PlayerIdentifyClass("foo")), serve_area);
        ...
        });
        {
            ...
            tes::geometry::Area2D serve_area{-15,-30,12,19};
            auto i = land_mng->prepareServe(tes::PlayerIdentify(new tes::PlayerIdentifyClass("foo")), serve_area);
            auto serve_land_id = land_mng->serve(std::get<0>(i));
            cpptf::isTrue("Land in mluti dominion #1", [=]() -> bool{
                auto& land = std::get<0>(land_mng->getLandByPos(serve_area.getStartPos())).get();
                return land.id == serve_land_id;
            });
            cpptf::isTrue("Land in mluti dominion #2", [=]() -> bool{
                auto& land = std::get<0>(land_mng->getLandByPos(serve_area.getEndPos())).get();
                return land.id == serve_land_id;
            });
    ...
    }
    {
        ...
        cpptf::isTrue("Load from DB #1", [=]() -> bool{
            auto land = land_mng->getLandByPos(serve_area.getStartPos());
            return std::get<tes::Types::land_area_ref>(land).get().land.id == 0;
        }());
    }
}
}
